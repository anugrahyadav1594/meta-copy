...
Wires the single Platform singleton according to MODE:
...
Redis cache is initialized once and shared through the Platform.
"""
...
class Platform:
    ...
    async def start(self) -> None:
        ...
        if self.settings.sharding_active and self.settings.shard_urls():
            ...
            # Default traffic path follows the deployment mode.
    ...
    def _build_canonical_repos(self, engine: object) -> None:
        self.user_repo = CanonicalUserRepository(engine)
        self.post_repo = CanonicalPostRepository(engine)
        self.comment_repo = CanonicalCommentRepository(engine)
...
async def platform_lifespan(app: object) -> object:
    ...
    async def lifespan(_app: object) -> AsyncIterator[None]:
        ...
        if (
            platform.sharding_available
            and platform.health_checker is not None
        ):
